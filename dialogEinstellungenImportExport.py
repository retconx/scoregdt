...
class EinstellungenImportExport(QDialog):
    def __init__(self, configPath):
        ...
        self.checkboxTextListe = ["Allgemein", "GDT-Einstellungen", "BenutzerInnen", "LANR/Lizenzschlüssel"]
    ...
    def accept(self):
        if self.buttonBox.button(QDialogButtonBox.StandardButton.Ok).text() == "Importieren...":
            ...
            fd.setModal(True)
            fd.setNameFilters(["ScoreGDT-Einstellungsdateien (*.sed)"])
            ...
            if fd.exec() == 1:
                ...
                if "Allgemein" in configImport.sections() or "GDT" in configImport.sections() or "Benutzer" in configImport.sections() or "Erweiterungen" in configImport.sections():
                    ...
                    try:
                        ...
                        mb = QMessageBox(QMessageBox.Icon.Warning, "Hinweis von ScoreGDT", "Fehler beim Importieren der Einstellungen: " + str(e), QMessageBox.StandardButton.Ok)
                        mb.exec()
                else:
                    mb = QMessageBox(QMessageBox.Icon.Warning, "Hinweis von ScoreGDT", "Die Datei " + datei + " ist keine gültige ScoreGDT-Konfigurationsdatei", QMessageBox.StandardButton.Ok)
                    mb.exec()
        else:
            fd = QFileDialog(self)
